...
START_TEST(logger_init_fail)
  void* mem = malloc(72);
  ASSERT(mem != nullptr);
  auto logger = logger_basic_create(mem, 72, stderr, LOG_LEVEL_DEBUG);
...
START_TEST(logger_init_success)
  ...
  ASSERT(mem != nullptr);
  auto logger = logger_basic_create(mem, logger_min_req_memory, stderr, LOG_LEVEL_DEBUG);
...
START_TEST(f_log_test)
  void* mem = malloc(logger_min_req_memory);
  ASSERT(mem != nullptr);
  auto logger = logger_basic_create(mem, logger_min_req_memory, stderr, LOG_LEVEL_INFO);
...
START_TEST(multy_sink_log)
  ...
  ASSERT(mem != nullptr);
  auto logger = logger_basic_create(mem, logger_min_req_memory + 50, stderr, LOG_LEVEL_WARN);
